...
const InteractiveQuiz = () => {
    ...
    const calculateResults = () => {
        let tempScores = { landing: 0, portfolio: 0, resume: 0 };
        let timeline = '';
        let budget = '';
        ...
        questions.forEach(q => {
            ...
            if (option && option.timeline) timeline = option.timeline;
            if (option && option.budget) budget = option.budget;
        });

        showResults(timeline, budget, tempScores);
    };

    const showResults = (timeline, budget, scores) => {
        ...
        setResultData({ resultType, recommendation, features, timelineText: timelineMap[timeline] || timelineMap.normal });
        setShowResult(true);
    };
            ...
            {!showResult && (
                <div className="InteractiveQuiz-quizScreen">
                    <div className="InteractiveQuiz-progressContainer">
                        <div className="InteractiveQuiz-progressBar" style={{ width: `${((currentQuestion + 1) / questions.length) * 100}%` }}></div>
                    </div>
                    <div className="InteractiveQuiz-progressText">Крок {currentQuestion + 1} з {questions.length}</div>

                    <div className="InteractiveQuiz-questionContainer">
                        <h3 className="InteractiveQuiz-questionTitle">{questions[currentQuestion].question}</h3>
                        <div className="InteractiveQuiz-options">
                            {questions[currentQuestion].options.map((opt, idx) => (
                                <div key={idx} className={`InteractiveQuiz-option ${answers[questions[currentQuestion].id] === opt.value ? 'InteractiveQuiz-selected' : ''}`} onClick={() => selectOption(questions[currentQuestion].id, opt.value)}>
                                    <input type="radio" name={`q${questions[currentQuestion].id}`} value={opt.value} checked={answers[questions[currentQuestion].id] === opt.value} readOnly />
                    ...
                    <div className="InteractiveQuiz-buttonGroup">
                        {currentQuestion > 0 && <button className="InteractiveQuiz-btn Secondary" onClick={prevQuestion}>Назад</button>}
                        <button className="InteractiveQuiz-btn Primary" onClick={nextQuestion} disabled={!answers[questions[currentQuestion].id]}>
                ...
                <div className="InteractiveQuiz-resultScreen">
                    <div className="InteractiveQuiz-resultCard">
                        ...
                        <ul className="InteractiveQuiz-featuresList">
                            {resultData.features && resultData.features.map((f, idx) => <li key={idx}>{f}</li>)}
                        </ul>
                        <div className="InteractiveQuiz-timelineInfo"><strong>{resultData.timelineText}</strong></div>
                        <div className="InteractiveQuiz-buttonGroup">
                            <button className="InteractiveQuiz-btn Secondary" onClick={restartQuiz}>Пройти знову</button>
                        </div>
                    </div>
                </div>
            )}
