...
const Pricing = () => {
    ...
    const togglePricing = () => setShowTable(!showTable);
    ...
    const totalPrice = basePrice + Object.values(checkboxValues).reduce((a, b) => a + b, 0);
    ...
    return (
        <section className="Pricing-section">
                    ...
                    {Object.keys(servicesData).map((category) => (
                        <div key={category} className="Pricing-serviceCategory">
                            <h3 className="Pricing-titleCategory">
                                {category === 'portfolio' ? 'Портфоліо' :
                                    category === 'resume' ? 'Резюме' :
                                        category === 'sites' ? 'Сайти' :
                                            'Адаптація сайту для смартфонів'}
                            </h3>
                            {servicesData[category].map((item, i) => (
            ...
            <div className="Pricing-calculator">
                ...
                {serviceType && (
                    <div className="Pricing-group">
                        ...
                        <select id="service-option" value={serviceOption} onChange={handleOptionChange}>
                            ...
                            {servicesData[serviceType].map((opt, i) => (
                ...
                {showAdditional && (
                    <div className="Pricing-group">
                        ...
                        <div className="Pricing-checkboxGroup">
                            <label>
                                <input type="checkbox" id="option-seo" value={200} checked={checkboxValues['option-seo'] > 0} onChange={handleCheckboxChange} />
                                SEO оптимізація (+200 грн)
                            </label>
                            <label>
                                <input type="checkbox" id="option-support" value={150} checked={checkboxValues['option-support'] > 0} onChange={handleCheckboxChange} />
                                Технічна підтримка 1 місяць (+150 грн)
                            </label>
                            <label>
                                <input type="checkbox" id="option-forms" value={100} checked={checkboxValues['option-forms'] > 0} onChange={handleCheckboxChange} />
                ...
                {showResult && (
                    <button className="Pricing-resetBtn" onClick={resetCalculator}>Очистити</button>
                )}
            </div>
        </section>
    );
